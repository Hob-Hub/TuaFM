-- Billboard Year-End Hot 100 SQLite schema
-- Annual chart data, separate from the weekly LOS40 database.
...
SELECT
    ...
    t.wikipedia_url         AS song_wikipedia_url,
    ye.source_url,
    NULL                    AS cover_url,
    NULL                    AS youtube_url,
    ...
    t.id                    AS billboard_track_id,
    ye.id                   AS billboard_entry_id
...
SELECT
    yc.chart_year,
    yc.chart_date,
    ye.source_artist_list AS artist,
    t.title               AS song,
    ye.source_url,
    NULL                  AS cover_url
FROM year_end_entries ye
